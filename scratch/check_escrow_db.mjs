...
async function main() {
    const { data: wallets, error: wError } = await supabase.from('professional_wallets').select('*, user:users(email)').order('created_at', { ascending: false }).limit(5);
    console.log("WALLETS:", wallets);

    const { data: escrows, error: eError } = await supabase.from('escrow_orders').select('*').limit(5);
