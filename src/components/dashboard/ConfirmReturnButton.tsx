...
export function ConfirmReturnButton({ orderId }: ConfirmReturnButtonProps) {
    ...
    const executeConfirmReturn = async () => {
        setShowModal(false);
        setLoading(true);
        setError(null);
        try {
    ...
    return (
        <>
            <div className="flex flex-col gap-1 w-full mt-2">
                {error && <span className="text-xs text-red-600 font-bold bg-red-50 p-1 rounded px-2">{error}</span>}
                <button
                    onClick={() => setShowModal(true)}
                    disabled={loading}
                    className={clsx(
                        "px-4 py-2 rounded-lg font-bold flex items-center justify-center gap-2 transition-all w-full text-xs",
                        loading 
