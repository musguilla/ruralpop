...
interface Props {
    params: { brand: string, province: string };
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
    const supabase = await createClient();
    const urlPath = `/tractores/${params.brand}/provincia/${params.province}`;
...
export default function Page({ params }: Props) {
    return <TractorCombinationPage urlPath={`/tractores/${params.brand}/provincia/${params.province}`} />;
}
