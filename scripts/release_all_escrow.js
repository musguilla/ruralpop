...
// We only need Equipop for this case.
const stripeEquipop = new Stripe(process.env.EQUIPOP_STRIPE_SECRET_KEY, { apiVersion: '2023-10-16' });
...
async function run() {
    ...
    for (const order of orders) {
        ...
        try {
            const stripe = stripeEquipop; // Force equipop
            ...
            const buyerObj = Array.isArray(order.buyer) ? order.buyer[0] : order.buyer;
            const sellerObj = Array.isArray(order.seller) ? order.seller[0] : order.seller;
            const buyerName = buyerObj?.name || buyerObj?.email || order.buyer_id;
            const sellerName = sellerObj?.name || sellerObj?.email || order.seller_id;
            
            details.push(`- **${(order.seller_net_amount_cents / 100).toFixed(2)}€** del comprador **${buyerName}** al vendedor **${sellerName}**`);
            console.log(`✅ Success`);
